...
export default function DeleteRecipeForm({recipe, cancelFunction} : {recipe: Recipe, cancelFunction : ()=>void}) {
	...
	for (let i = 0; i < recipe.title.length; i++) {
		if(recipe.title.length > 25 && i > 0 && i >= prevIndex + numberOfCharactersBetweenSpaces) {
			const stringToCheck = recipe.title.slice(i-1, i+1);
			if (!stringToCheck.includes(" ") && !stringToCheck.includes("-")) title += `-`;
			prevIndex = i;
		} 
	...
	return (
		<form action={formAction}>
			<div className="rounded-md border-[2px] border-gray-300 min-w-[200px] w-[35%] m-auto bg-gray-100 p-4 md:p-6">
				...
				<div className="">
					<h1 className="max-md:text-sm text-center">Are you sure you want to delete this recipe?</h1>
