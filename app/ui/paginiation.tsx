...
export default function Pagination({currentPage, recipesPerPage, recipes}:{currentPage: number, recipesPerPage: number, recipes: RowList<Recipe[]>}) {
	const params = useSearchParams();

	function pageHref(page : number) {
		return `recipes?page=${page}${params.get("query") ? `&query=${params.get("query")}` : ""}`;
	}
	function PageNumberButton({page} : {page: number}) {
		const selectedClass = "bg-[#B6BCC5] w-8 h-8 text-lg text-center border-gray-500 border-2";
		const notSelectedClass = "bg-gray-300 w-8 h-8 text-lg text-center border-gray-400 border-2 hover:bg-[#B6BCC5] hover:border-gray-500";

		return (
			<Link
				href={pageHref(page)}
				className={page == currentPage ? selectedClass : notSelectedClass}
				>
				{page}
			</Link>
		) as JSX.Element
	...
	const recipesToDisplay : Recipe[] = currentPage >= 1 ? recipes.slice(amountToSkip, Math.min(amountToSkip + recipesPerPage, recipes.length)) : [];
		...
		<>
			{recipesToDisplay?.map((recipe) => {
				return (
					<div key={recipe.id}>
						...
						{recipe.id != recipesToDisplay.at(-1)?.id &&
							<hr />
						}
					</div>
				);
			...
			{recipesToDisplay.length <= 0 || currentPage < 1 ? (
				...
				<>
					...
					<Link
						href={pageHref(currentPage < 1 ? 1 : pagesCount)}
						className="underline text-blue-600 hover:text-blue-800"
					> 
						Click here to go to the {currentPage < 1 ? "first" : "last"} page.
					</Link>
				</> 
				: <>
					<h1>Sorry, no recipes found with that search. :/</h1>
				</>
				}
